...
public class UserSkill_MianLiCangZhen : SkillBase
{
    ...
    public override string Des => "绵里藏针:你传出的情报被接收后，可以将一张黑色手牌置入接收者的情报区，然后摸一张牌\n";
    ...
    public static void OnReceiveUse(int playerId, int targetPlayerId, CardFS card)
    {
        ...
        string s = string.Format("{0}使用了技能绵里藏针,将一张{1}置入{2}情报区", GameManager.Singleton.players[playerId].name, card.GetCardInfo(), GameManager.Singleton.players[targetPlayerId].name);
